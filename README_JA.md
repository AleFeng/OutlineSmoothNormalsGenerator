![Outline Smooth Normals Generator バナー](./Packages/com.alefeng.outlinesmoothnormalsgenerator/Docs~/Images/banner.png)
...
![左：通常法線アウトライン（ハードエッジで割れる）／右：スムース法線アウトライン（連続して閉じる）](./Packages/com.alefeng.outlinesmoothnormalsgenerator/Docs~/Images/comp_cube.png)

## 📜 目次
- [Outline Smooth Normals Generator - スムース法線アウトライン生成ツール](#outline-smooth-normals-generator---スムース法線アウトライン生成ツール)
  - [📜 目次](#-目次)
  - [概要](#概要)
    - [特徴](#特徴)
  ...
  - [📦 インストール](#-インストール)
    ...
    - [アウトラインシェーダーのインポート（必須）](#アウトラインシェーダーのインポート必須)
    - [その他の方法](#その他の方法)
  - [🚀 クイックスタート](#-クイックスタート)
    - [1. ツールを開く](#1-ツールを開く)
...
![メインウィンドウ：左にパラメーター、右にデータ概要とリアルタイムプレビュー](./Packages/com.alefeng.outlinesmoothnormalsgenerator/Docs~/Images/tool_generate.png)

### 特徴
| 特徴 | 説明 |
| --- | --- |
| 角度重み付けスムース法線 | 頂点を「位置が等しい」ことでグループ化し、面法線を角度で重み付けして平均。ハードエッジをまたぐ連続した押し出し方向を生成し、アウトラインの割れを根本から解消します。背面・両面メッシュの巻き順の向きも自動補正します。 |
| 3 つの保存方式 | **頂点カラー**（RG / GB / BA のペアを選択可、八面体エンコード）、**接線チャンネル**（`tangent.xyz`）、**TEXCOORD0–7**（8 チャンネル、2 成分の八面体エンコード、2 float / 頂点）。いずれも完全な三次元方向を格納し、八面体エンコードは全球面の全単射なので符号の曖昧さがありません。 |
| 2 つの保存空間 | 「どのチャンネルに書くか」とは**直交**する軸。**接線空間**（既定）は各頂点自身の TBN を基準とした座標を格納し、デコード時に**スキニング後**の法線と接線から TBN を再構築するため、`SkinnedMeshRenderer` でもアウトラインが裂けません。接線は「基底」として使うだけで**占有しない**ので、ノーマルマップはそのまま使えます。 |
| インポート時の自動ベイク | 判定ルール —— **ファイル名の接尾辞**（既定 `_Outline`）と**フォルダーパス**、両方を有効にすると積集合 —— に一致するモデルを、（再）インポート時に自動でスムース法線をベイク。**非破壊的**で手動操作は不要。ツールの「インポート自動ベイク」タブで設定（`ProjectSettings/` に保存）。カスタム判定ルール／カスタム保存の 2 つの拡張デリゲートも提供します。 |
| メッシュ健全性チェック | 生成／ベイク前に、法線欠落・退化三角形・NaN・同一位置への頂点重複過多・Read/Write 無効、および接線空間保存時の接線の不正（欠落・ゼロベクトル・法線と平行）を走査して報告。エラー時は生成前に再確認、自動ベイク時は自動スキップします。 |
...
| Scene ビューへの法線オーバーレイ | 同じ 2 種類の線分を実際のシーンのオブジェクト上へ描画します。`SkinnedMeshRenderer` は**現在のポーズ**で評価されるため、アニメーション再生中に接線空間のデータを検証できます。頂点数が多い場合は自動で間引き、その間引き率をパネルに明示します。 |
...
- 「保存方式」で **頂点カラー／接線チャンネル／TEXCOORD** を選びます（[3 つの保存方式](#-3-つの保存方式)を参照）。右側の「データチャンネル状態の一覧」で、対象チャンネルに既にデータがあるかを確認できます。
- 「保存空間」で **オブジェクト空間／接線空間** を選びます（[保存空間](#-保存空間)を参照）。既定は**接線空間**で、スキンメッシュでは必須です。⚠ 「保存方式」と「保存空間」は、どちらも後でマテリアル側で**同じ設定**にする必要があります。食い違ってもエラーは出ず、アウトラインがずれるだけです。
...
## 📥 インポート時の自動ベイク
上記の手動フローに加えて、ツールは**インポート時に自動ベイク**できます。モデルのファイル名を既定の接尾辞（既定 `_Outline`、例：`Hero_Outline.fbx`）にする、あるいは指定したフォルダーへ置いておくと、（再）インポートした瞬間にスムース法線がメッシュへ書き込まれます —— ツールを開く必要も、独立した Mesh として保存する必要もありません。**非破壊的**で、判定に一致しないようにするかスイッチをオフにして再インポートすれば元のメッシュに戻ります。

ツールウィンドウ上部の **「インポート自動ベイク」タブ** で有効化と設定を行います：有効化スイッチ、判定ルール（**ファイル名の接尾辞**と**フォルダーパス**。それぞれ個別に有効化でき、両方を有効にすると積集合になります）、保存方式（頂点カラー／接線チャンネル／`TEXCOORD0`–`7`）、保存空間（オブジェクト空間／接線空間、既定は接線空間）、結合トレランス。設定は `ProjectSettings/OutlineSmoothNormals.asset` に保存され、プロジェクトと一緒にバージョン管理されるため、チームで同じ設定を共有できます。

![インポート自動ベイクタブ](./Packages/com.alefeng.outlinesmoothnormalsgenerator/Docs~/Images/tool_auto.png)

生成／ベイクの前には**メッシュ健全性チェック**が走ります（法線欠落・退化三角形・NaN・同一位置への頂点重複過多など）。問題は即座に報告され、エラーのメッシュは自動的にスキップされます。フォルダー／ラベルで自社パイプラインに組み込みたい場合や、カスタム保存フォーマットを使いたい場合は、2 つの拡張デリゲートで引き取れます —— 詳しくは[詳細ドキュメント](Packages/com.alefeng.outlinesmoothnormalsgenerator/README.md#导入时自动烘焙)を参照してください。

## 🧩 3 つの保存方式
スムース法線は常に**完全な三次元方向**として格納されます。半球圧縮を行わないため、符号の曖昧さがなく、ハードエッジの角でもデコードを誤りません。ここで決めるのは「どのチャンネルに書くか」で、「どの空間で格納するか」は[保存空間](#-保存空間)が受け持ちます。

| モード | 格納先 | 適した場面 |
| --- | --- | --- |
| **頂点カラー Vertex Color** | `color` の RG / GB / **BA**（既定）ペア、八面体エンコード | 頂点カラーが空いている場合の第一候補。8-bit × 2 成分、誤差は約 1°。 |
| **接線チャンネル Tangent Channel** | `tangent.xyz`（`w` は常に 1） | float の完全な精度。⚠ **元の接線を上書きしノーマルマップを壊します** —— そのメッシュがノーマルマップを使わない場合のみ選んでください。 |
| **TEXCOORD** | `TEXCOORD0`–`TEXCOORD7` の `xy`、八面体エンコード | 頂点カラーが使用済みの場合の推奨、8 チャンネル。2 成分の八面体エンコード（2 float / 頂点）、誤差は約 5e-6°。 |
...
> ⚠️ **`1.6.x` 以前からのアップグレードにおける破壊的変更**：`1.7.0` から TEXCOORD チャンネルは **2 成分の八面体エンコード**で格納されます（それ以前は 3 成分の生の方向）。`1.6.x` 以前にベイクしたデータはもうデコードできないため、**ベイクし直す**必要があります。頂点カラーと接線チャンネルでの保存はバイト単位で変わっておらず、影響を受けません。詳しい移行手順は[詳細ドキュメント](Packages/com.alefeng.outlinesmoothnormalsgenerator/README.md#从-16x-及更早升级)を参照してください。

## 🧭 保存空間
「どのチャンネルに書くか」（保存方式）とは**直交**するもう一つの軸で、方向そのものを**どの空間で**格納するかを決めます。

| 保存空間 | 格納する内容 | 適した対象 |
| --- | --- | --- |
| **オブジェクト空間 Object Space** | バインドポーズにおけるオブジェクト空間の方向。デコードすればそのまま使えます | 静的モデルのみ |
| **接線空間 Tangent Space** | 各頂点自身の TBN を基準とした座標。デコード時に**スキニング後**の法線と接線から TBN を再構築します | 静的モデルとスキンモデルの両方（**既定**） |

`SkinnedMeshRenderer` のスキニングでは、Unity は **POSITION / NORMAL / TANGENT** を変換しますが、**COLOR と TEXCOORD はそのまま渡され、スキニングされません**。そのため頂点カラーや TEXCOORD に入れたオブジェクト空間の方向は「頂点はボーンに追従するのに、押し出し方向だけがバインドポーズに取り残される」状態になり、関節を曲げた途端にアウトラインが裂けます。  
接線空間の座標は**スキニング不変量**です —— `N` と `T` も一緒に変換されるため、`S = a·T + b·B + c·N` の両辺に同じ回転がかかり、`(a, b, c)` は変わりません。ノーマルマップがボーンアニメーション上で正しく機能するのと同じ理屈です。

- **接線は占有しません** —— ここでの接線はあくまで「基底」であって格納先ではないため、ノーマルマップはそのまま使えます。条件はモデルのインポート設定で `Tangents` が `None` でないことだけです。
- **接線チャンネル保存には適用されません**。基底の再構築に必要な接線自体を上書きしてしまううえ、そもそも不要です —— Unity は `tangent.xyz` を方向として一緒にスキニングするからです。このモードではツール側もマテリアルパネルも自動でグレーアウトされます。
- **精度は変わりません**。エンコードするのは単位方向のままなので、頂点カラーでも誤差は約 1° です。
- ⚠ **マテリアル側の `Smooth Normal Space` を必ず同じに設定してください。** どちらの空間も単位方向を格納するだけでデータからは区別できず、間違えてもエラーは出ません。アウトライン全体が傾くだけです。

> ⚠️ **`1.4.x` からのアップグレード**：`1.5.0` から保存空間の既定は接線空間になり、マテリアルの **Smooth Normal Space** も既定が `Tangent Space` になりました。一方 `1.4.x` 以前にベイクしたデータはすべてオブジェクト空間です —— **一度ベイクし直す**か、マテリアルのこの項目を `Object Space` に戻してください。[インポート時の自動ベイク](#-インポート時の自動ベイク)を使っているモデルは自動で再ベイクされるため、対応は不要です。詳しくは[詳細ドキュメント](Packages/com.alefeng.outlinesmoothnormalsgenerator/README.md#存储空间)を参照してください。
...
3. **Smooth Normal Space** を、生成時と同じ[保存空間](#-保存空間)に設定します（既定は `Tangent Space`）。⚠ ここを間違えても**エラーは出ず**、アウトライン全体が傾くだけです。アウトラインの異常を調べるときは、まずこの項目を確認してください。
4. アウトラインの色と幅を調整します。**幅モード**は **スクリーンスペース**（等幅・距離に依存しない）または **ワールドスペース**（ワールド単位でオフセットし、距離とともに縮小）から選べます。
