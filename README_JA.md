...
| 3 つの保存方式 | **頂点カラー**（RG / GB / BA のペアを選択可、八面体エンコード）、**接線**（`tangent.xyz`）、**TEXCOORD0–3**。いずれもオブジェクト空間の完全な三次元方向を格納し、圧縮による曖昧さがありません。 |
...
> ⚠️ **`.fbx` などのモデルファイル内のメッシュは読み取り専用のインポート済みサブアセット**です —— 書き込んだデータは次回の再インポート時に失われます。ツールはこれを検出して保存を阻止するので、まず **`⧉ 独立した Mesh として保存…`** で書き込み可能な `.asset` を複製してください。複製されたメッシュはオブジェクトへ自動的に再割り当てされます。
>
...
| **TEXCOORD** | `TEXCOORD0`–`TEXCOORD3` の `xyz` | 頂点カラーが使用済みの場合の推奨。float の完全な精度。 |
