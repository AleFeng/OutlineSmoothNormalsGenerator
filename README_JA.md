...
| 3 つの保存方式 | **頂点カラー**（RG / GB / BA のペアを選択可、八面体エンコード）、**接線チャンネル**（`tangent.xyz`）、**TEXCOORD0–7**（8 チャンネル）。いずれも完全な三次元方向を格納し、圧縮による曖昧さがありません。 |
...
## 📥 インポート時の自動ベイク
上記の手動フローに加えて、ツールは**インポート時に自動ベイク**できます。モデルのファイル名を既定の接尾辞（既定 `_Outline`、例：`Hero_Outline.fbx`）にしておくと、（再）インポートした瞬間にスムース法線がメッシュへ書き込まれます —— ツールを開く必要も、独立した Mesh として保存する必要もありません。**非破壊的**で、接尾辞を外すかスイッチをオフにして再インポートすれば元のメッシュに戻ります。

ツールウィンドウ上部の **「インポート自動ベイク」タブ** で有効化と設定を行います：有効化スイッチ、判定用の接尾辞、保存方式（頂点カラー／接線チャンネル／`TEXCOORD0`–`7`）、保存空間（オブジェクト空間／接空間、既定は接空間）、結合トレランス。設定は `ProjectSettings/OutlineSmoothNormals.asset` に保存され、プロジェクトと一緒にバージョン管理されるため、チームで同じ設定を共有できます。
