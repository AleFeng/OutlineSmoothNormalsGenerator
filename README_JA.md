...
| 2 つの保存空間 | 「どのチャンネルに書くか」とは**直交**する軸。**接空間**（既定）は各頂点自身の TBN を基準とした座標を格納し、デコード時に**スキニング後**の法線と接線から TBN を再構築するため、`SkinnedMeshRenderer` でもアウトラインが裂けません。接線は「基底」として使うだけで**占有しない**ので、ノーマルマップはそのまま使えます。 |
...
| メッシュ健全性チェック | 生成／ベイク前に、法線欠落・退化三角形・NaN・同一位置への頂点重複過多・Read/Write 無効、および接空間保存時の接線の不正（欠落・ゼロベクトル・法線と平行）を走査して報告。エラー時は生成前に再確認、自動ベイク時は自動スキップします。 |
...
| Scene ビューへの法線オーバーレイ | 同じ 2 種類の線分を実際のシーンのオブジェクト上へ描画します。`SkinnedMeshRenderer` は**現在のポーズ**で評価されるため、アニメーション再生中に接空間のデータを検証できます。頂点数が多い場合は自動で間引き、その間引き率をパネルに明示します。 |
...
- 「保存空間」で **オブジェクト空間／接空間** を選びます（[保存空間](#-保存空間)を参照）。既定は**接空間**で、スキンメッシュでは必須です。⚠ 「保存方式」と「保存空間」は、どちらも後でマテリアル側で**同じ設定**にする必要があります。食い違ってもエラーは出ず、アウトラインがずれるだけです。
...
ツールウィンドウ上部の **「インポート自動ベイク」タブ** で有効化と設定を行います：有効化スイッチ、判定ルール（**ファイル名の接尾辞**と**フォルダーパス**。それぞれ個別に有効化でき、両方を有効にすると積集合になります）、保存方式（頂点カラー／接線チャンネル／`TEXCOORD0`–`7`）、保存空間（オブジェクト空間／接空間、既定は接空間）、結合トレランス。設定は `ProjectSettings/OutlineSmoothNormals.asset` に保存され、プロジェクトと一緒にバージョン管理されるため、チームで同じ設定を共有できます。
...
| **接空間 Tangent Space** | 各頂点自身の TBN を基準とした座標。デコード時に**スキニング後**の法線と接線から TBN を再構築します | 静的モデルとスキンモデルの両方（**既定**） |
...
接空間の座標は**スキニング不変量**です —— `N` と `T` も一緒に変換されるため、`S = a·T + b·B + c·N` の両辺に同じ回転がかかり、`(a, b, c)` は変わりません。ノーマルマップがボーンアニメーション上で正しく機能するのと同じ理屈です。
...
> ⚠️ **`1.4.x` からのアップグレード**：`1.5.0` から保存空間の既定は接空間になり、マテリアルの **Smooth Normal Space** も既定が `Tangent Space` になりました。一方 `1.4.x` 以前にベイクしたデータはすべてオブジェクト空間です —— **一度ベイクし直す**か、マテリアルのこの項目を `Object Space` に戻してください。[インポート時の自動ベイク](#-インポート時の自動ベイク)を使っているモデルは自動で再ベイクされるため、対応は不要です。詳しくは[詳細ドキュメント](Packages/com.alefeng.outlinesmoothnormalsgenerator/README.md#存储空间)を参照してください。
