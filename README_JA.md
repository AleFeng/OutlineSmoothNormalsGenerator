...
ツールウィンドウ上部の **「インポート自動ベイク」タブ** で有効化と設定を行います：有効化スイッチ、判定用の接尾辞、保存方式（頂点カラー／接線／`TEXCOORD0`–`7`）、結合トレランス。設定は `ProjectSettings/OutlineSmoothNormals.asset` に保存され、プロジェクトと一緒にバージョン管理されるため、チームで同じ設定を共有できます。

![インポート自動ベイクタブ](./Packages/com.alefeng.outlinesmoothnormalsgenerator/Docs~/Images/tool_auto.png)

生成／ベイクの前には**メッシュ健全性チェック**が走ります（法線欠落・退化三角形・NaN・同一位置への頂点重複過多など）。問題は即座に報告され、エラーのメッシュは自動的にスキップされます。フォルダー／ラベルで自社パイプラインに組み込みたい場合や、カスタム保存フォーマットを使いたい場合は、2 つの拡張デリゲートで引き取れます —— 詳しくは[詳細ドキュメント](Packages/com.alefeng.outlinesmoothnormalsgenerator/README.md#导入时自动烘焙可选)を参照してください。
