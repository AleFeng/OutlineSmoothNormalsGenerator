...
- **场景对象** —— 含 `MeshFilter` / `SkinnedMeshRenderer`（只取该对象自身的渲染器，不递归子物体）。
- **Mesh 资产** —— Project 里的独立 `.asset`，或展开 FBX 选中的 Mesh 子资产。
- **模型**（`.fbx` / `.obj` 等）**或预制体** —— 自动遍历层级，收集其中全部网格；含多个网格时，
  用目标区的 **Mesh 下拉**逐个选择（同一网格被多个渲染器共用时自动去重）。
